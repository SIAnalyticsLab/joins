...
output:
  html_document:
    ...
    toc: true
    toc_float: true
    css: !expr here::here("global/style/si-analytics.css")
    highlight: kate
editor_options: 
...
We will learn how to create factors and how to manipulate them with functions from the `forcats` package, a part of the tidyverse.
...
-   We will use a dataset on **HIV mortality** in Colombia from 2010 to 2016
    ...
    -   You can learn more [here](https://www.datos.gov.co/en/Salud-y-Protecci-n-Social/Mortalidad-VIH-2010-A-2016/yht4-twf4).
...
-   A categorical variable takes on a limited set of possible values or levels.
    -   Examples: country, race or political affiliation.
-   These differ from free-form string variables like person names or book titles.
-   Factors have advantages over character vectors:
    -   More efficient storage in R

    -   Some statistical functions like `lm()`, require categorical variables to be input as factors

    -   Control over the order of categories or levels.

## Factors in Action

-   Interested in visualizing the patients in the dataset by their birth month?
...
-   **Solution:** Creating a factor using the `factor()` function:

```{r}
hiv_mort_modified <- 
  hiv_mort %>% 
  mutate(birth_month = factor(x = ______, 
                             levels = c(______, ______, ______, ______,
                                        ______, ______, ______, ______,
                                        ______, ______, ______, ______)))
```

-   `x` argument takes the original character column, `birth_month`

-   `levels` argument takes in the desired sequence of months.
...
-   Regenerate the ggplot with the modified dataset:
...
-   For example, compare how the `count()` function displays frequency tables:
...
-   `forcats` offers a set of functions that make factor manipulation simpler

-   We'll explore four functions, but there are many others, we encourage you to explore the *forcats* website on your own time [here](https://forcats.tidyverse.org/reference/index.html)!

## fct_relevel

-   `fct_relevel()` function is used to manually change the order of factor levels. For example, to visualize the frequency of individuals by municipality type:
...
hiv_mort_pop_center_first <- 
  hiv_mort %>% 
  mutate(municipality_type = fct_relevel(______,______))
```

-   Syntax: we pass the **factor variable as the [first argument]{.underline}**, and the **level as the [second argument]{.underline}**.
...
-   specify:
    ...
    -   and use the `after` argument to define the position

-   We can also [**move multiple levels**]{.underline} at a time by providing these levels to `fct_relevel()`:
...
summary_per_muni_reordered <- 
  summary_per_muni %>%
  mutate(municipality_name = fct_reorder(.f = ______, 
                                         .x = ______))
```
...
hiv_mort_muni_recode <- hiv_mort %>%
  mutate(municipality_type = fct_recode(______, 
                                        ______ = ______))
...
hiv_mort_lump_muni <- hiv_mort %>%
  mutate(municipality_name = fct_lump(______, ______))
...
# Well done {.unlisted .unnumbered}

You can now submit your code along 
