...
pacman::p_load(tidyverse, 
               RColorBrewer,
               knitr, 
               here, janitor,  reactable, gt, flextable)
...
load(here::here("data/clean/nigerm_cases_rgn.RData"))
```
...
reactable::reactable(head(nigerm, 10))
```
...
The `select()` and `filter()` functions are part of the {dplyr} package for data manipulation, which is a core package of the {tidyverse}. These topics are covered in the Data Wrangling course. See The SI Analytics [website](https://sianalytics.org/) for more.
:::
...
#| eval: true
## Print nigerm96
reactable::reactable(head(nigerm96, 20))
